...
func build_corona() -> void:
	...
	add_child(corona)
	var light := S.mat(Color('#ffd15a'),true)
	light.emission_energy_multiplier = 3.8
	var circle := S.ring(corona,Vector3.ZERO,.85,light)
	circle.mesh.inner_radius = .833
	circle.rotation.x = PI/2
	for i in range(12):
		var angle := TAU*i/12.0
		var mark := S.box(corona,Vector3(sin(angle)*.91,cos(angle)*.91,0),Vector3(.022,.09,.018),light)
...
func build_particles() -> void:
	...
	fade.colors = PackedColorArray([Color(1,.8,.3,0),Color(1,.9,.6,1),Color(1,.64,.14,.7),Color(1,.45,.05,0)])
	var ramp := GradientTexture1D.new()
	ramp.gradient = fade
	process.color_ramp = ramp
	particles.process_material = process
	var quad := QuadMesh.new()
	quad.size = Vector2(.095,.095)
	...
	material.emission = Color('#ffdc85')
...
func _process(delta: float) -> void:
	...
	corona.rotation.z = sin(clock*.4)*.08
