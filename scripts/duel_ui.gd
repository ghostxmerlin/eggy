...
static func draw(h) -> void:
	...
	if game.screen == 'duel_lobby':
		...
		h.txt('准备进入决斗',Vector2(172,211),25,h.TEAL,true)
		h.txt('1V1 · 对战人机 · 生命归零判负',Vector2(826,177),22,h.TEAL)
		var catalog = preload('res://scripts/class_catalog.gd')
		h.centered(catalog.NAMES[duel.selected_class]+' · 你的固定职业',720,280,32,h.INK,true)
		for j in range(5):
			var data: Array = catalog.SKILLS[duel.selected_class][j]
			h.txt(str(j+1)+'  '+data[1]+'：'+data[4],Vector2(210,341+j*49),24,h.INK)
		h.txt('双方 100 生命 · 职业技能与天赋沿用角色存档',Vector2(210,621),22,h.TEAL)
		...
		h.centered('Enter 开始决斗',606,729,17,h.INK)
		return
	...
	h.txt('五个职业专属技能',Vector2(1065,838),17,h.TEAL)
	h.txt('按 1—5 施放 · 留意控制状态',Vector2(1065,863),17,h.TEAL)
	h.draw_skills()
	if duel.phase == 'countdown' and not game.paused:
		...
		h.button('duel_room',Rect2(737,457,223,58),'查看技能',false)
...
static func health_bar(h, rect: Rect2, title: String, id: int, color: Color) -> void:
	...
	var bar := Rect2(rect.position+Vector2(20,54),Vector2(rect.size.x-40,23))
	...
	var actor = h.game.racers[id]
	h.txt(actor.skills.career.fx.status_text(actor.skills.career.states),rect.position+Vector2(20,98),16,h.TEAL)
