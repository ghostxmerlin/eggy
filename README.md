...
岛上点击右侧 **决斗场** 或按 **J**，先选择战士、法师、刺客或猎人，再进入玩家对一名人机的 1V1。双方各 100 生命，倒计时结束开战，生命归零判负；结算可再战、重新选择职业或返回岛屿。

决斗保留五个技能位：**1、2 号职业专属技能待定**，暂不可用，移除滚动（含 Shift）和飞扑；**3 咸鱼棒**每次实际命中扣 20 生命，**4 冰锥术**冻结，**5 破胆怒吼**恐惧。首版各职业共用生命和基础技能，具体职业招式后续确定。人机会接近、绕行、追击并配合控制技能出手。已穿皮肤与黄金机甲的粉色激光剑外观保留，皮肤不提供数值加成。

W/S、Q/E、A/D、鼠标转向和空格跳跃沿用原操作；Esc 暂停，暂停菜单可重新决斗或返回岛屿。决斗中 T 复位及竞速随机道具不可用。详见 [决斗场说明](docs/duel.md)。
...
岛上点击“参赛”或在暂停菜单参赛，进入 1 名玩家 + 31 名人机的巅峰赛。每局在 32 个起跑位中随机分配玩家和人机的位置，重开重新分配；玩家也可能在前排。3 秒倒计时后统一出发，前 24 名晋级，限时 150 秒。重开保留 32 名选手；返回岛屿会移除比赛人机，再次参赛重新组队。岛上 T 回到广场，比赛中 T 返回检查点，Esc 暂停。
...
| G | 岛上打开盲盒工坊 |
| B | 岛上打开衣柜 |
| J | 岛上进入决斗场职业选择；决斗中 1、2 号改为待定职业技能 |
| F3 | 显示性能面板 |
| F12 | 保存当前画面 |
