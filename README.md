...
岛上按 **C** 打开职业与天赋。第一次选定战士、法师、刺客或猎人后，职业绑定当前本地角色存档；岛屿、巅峰赛、决斗场使用同一套五个技能。参赛与决斗入口会引导尚未选职业的角色完成创建。原有皮肤、蛋币与收藏保留。
...
猎人没有宠物或召唤。各职业使用不同的伤害、控制和移动手段；没有强制配置回血。职业角色不再使用通用滚动/飞扑，Shift 也不触发旧技能。天赋每行二选一，共三行，分别取舍伤害/出手频率、控制/抗控、移动距离/移动冷却；只在对局外修改，不替换职业技能。
...
W/S、Q/E、A/D、鼠标转向和空格跳跃沿用原操作；Esc 暂停，暂停菜单可重新决斗或返回岛屿。决斗中 T 复位及竞速随机道具不可用；巅峰赛仍按冲线判胜，职业攻击主要造成短暂干扰，不启用生命淘汰。详见 [决斗场说明](docs/duel.md)。
...
选定职业后，1—5 使用上表的职业技能，Shift 禁用；下表旧五技能仅供尚未创建职业的岛屿练习和旧场景回归。
