...
用户先要求单人操作调试；**默认是岛屿自由活动及单人练习**，没有比赛限时和淘汰。早期的 32 人模式保留为 1 名玩家 + 31 名 AI、前 24 名晋级、150 秒限时，但不是当前重点，也不是联网多人。
...
"$GODOT_BIN" --path . -- --race              # 保留的 32 人 AI 模式入口
...
| 完整 AI 比赛 | `test_race.gd`，帧上限至少 12000；此测试自行设置非练习模式 |

完整单人路线测试也应给至少 12000 帧。`test_plane.gd` 自行使用 4 倍时间和 240 Hz 物理加速逻辑测试，不能用其耗时评价渲染性能。
